...
# file content is usually compressed.  This module contains rules for
# converting the XML into markdown, for subsequent processing.
#
...
$(gendir)/%.txt:	%.oo3
	$(ECHO_TARGET)
	$(MKDIR) $(gendir)
	gzcat -cf $*.oo3/contents.xml | xsltproc $(OOFLAGS) $(OO_MD_XSL) - > $@

#
# build: --Create HTML documents from OO_SRC.
#
build:	build-omnioutliner
build-omnioutliner:	$(OO_SRC:%.oo3=$(gendir)/%.txt)
...
clean-omnioutliner:
	$(ECHO_TARGET)
	$(RM) $(OO_SRC:%.oo3=$(gendir)/%.txt) $(OO_SRC:%.oo3=%.html) $(OO_SRC:%.oo3=%.pdf)
