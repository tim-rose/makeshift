...
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:oo="https://www.omnigroup.com/namespace/OmniOutliner/v3" xmlns="http://www.w3.org/1999/xhtml" exclude-result-prefixes="oo" version="1.0">
  ...
  <xsl:template match="oo:item" mode="meta">
    ...
    <xsl:if test="translate($title,'ABCDEFGHIJKLMNOPQRSTUVWXYZ',    'abcdefghijklmnopqrstuvwxyz') = 'metadata'">
  ...
  <xsl:template match="oo:item" mode="headerlevel">
    <xsl:variable name="title" select="normalize-space(oo:values/oo:text/oo:p)"/>
    <xsl:if test="translate($title,'ABCDEFGHIJKLMNOPQRSTUVWXYZ',    'abcdefghijklmnopqrstuvwxyz') = 'headerlevel'">
  ...
  <xsl:template match="/oo:outline/oo:root/oo:item[count(following-sibling) = 0]">
    ...
    <xsl:variable name="title" select="normalize-space(oo:values/oo:text/oo:p)"/>
    <xsl:if test="not(translate($title,'ABCDEFGHIJKLMNOPQRSTUVWXYZ',    'abcdefghijklmnopqrstuvwxyz') = 'metadata')">
      <xsl:if test="not(translate($title,'ABCDEFGHIJKLMNOPQRSTUVWXYZ',     'abcdefghijklmnopqrstuvwxyz') = $ignorecolumn)">
